...
export class SpeakerController {
  ...
  async findOne(@Headers('authorization') authHeader: string, @Param("id") speakerID: string, @Res() res: Response): Promise<object>{
    try{
  ...
  @Post()
    @UseGuards(MiddlewareApp)
    async create(@Headers('authorization') authHeader: string, @Body() speaker: Speaker, @Res() res: Response): Promise<Speaker | object> {
    try {
      let token = authHeader.split(' ')[1];
      let userFound = await this.authService.validate(token);
  ...
  @Put(':id')
  @UseGuards(MiddlewareApp)
  async update(@Headers('authorization') authHeader: string, @Param("id") speakerID: string, @Body() speaker: Speaker, @Res() res: Response): Promise<object> {
    try{
      let token = authHeader.split(' ')[1];
      let userFound = await this.authService.validate(token);
  ...
  @Delete(':id')
  @UseGuards(MiddlewareApp)
  async delete(@Headers('authorization') authHeader: string, @Param("id") speakerID: string,  @Res() res: Response): Promise<object> {
    try{
      let token = authHeader.split(' ')[1];
      let userFound = await this.authService.validate(token);
